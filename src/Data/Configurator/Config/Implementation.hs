...
foldPlan empty_ union_ lookup_ = loop
  where
    ...
    loop key  (Union       pl1 pl2) = loop key pl1 `union_` loop key pl2
    loop key  (ConfigPlan  a      ) = lookup_ key a
    loop _key  Empty                = empty_
{-# INLINE foldPlan #-}
...
subassocs key c = filter pred_ (subassocs' key c)
  where
    pred_ (name,_) = case stripPrefix key name of
...
subassocs_ subassocs_' = loop
  where
    ...
    loop !key (ConfigPlan map_) = subassocs_' key map_

submap :: Text -> CritBit Text a -> CritBit Text a
submap key map_
    | T.null key = map_
    | otherwise  = let (_ , gt) = CB.split (key <> ".")  map_
                       (lt, _ ) = CB.split (key <> ".~") gt
                    in lt

subassocsMap :: Text -> CritBit Text a -> [(Text, a)]
subassocsMap key map_ = CB.assocs (submap key map_)
...
nullSubmap key map_ =
    if T.null key
    then CB.null map_
    else case CB.lookupGT key map_ of
           Nothing -> True
           Just (key', _) ->
...
subgroups = loop
  where
    ...
    loop !key (ConfigPlan map_) = subgroupsMap key map_

subgroupsMap :: Text -> CritBit Text a -> [Text]
subgroupsMap pre_ map_ = loop (CB.lookupGT pre map_)
  where
    ...
    loop (Just (key,_)) =
        case T.stripPrefix pre key of
          ...
          Just sfx -> let (sfxa, sfxz) = T.break ('.' ==) sfx
                       in if T.null sfxz
                          then loop (CB.lookupGT key map_)
                          else let key' = pre <> sfxa
                                in key' : loop (CB.lookupGT (key' <> "/") map_)
